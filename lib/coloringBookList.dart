...
import 'rotation3d.dart';


class ColoringBookList extends StatefulWidget {
  final List<Art> arts;
  final Function onArtChange;

  const ColoringBookList({Key key, this.arts, @required this.onArtChange}) : super(key: key);

  @override
  ColoringBookListState createState() => ColoringBookListState();
}

class ColoringBookListState extends State<ColoringBookList> with SingleTickerProviderStateMixin {
...
//hitron CEA 3
  ...
  Widget build(BuildContext context) {
    ...
    _cardHeight = (size.height * .66).clamp(300.0, 400.0);
  ...
  bool _handleScrollNotifications(Notification notification) {
    ...
    if (notification is ScrollUpdateNotification) {
      ...
      widget.onArtChange(widget.arts.elementAt(_pageController.page.round() % widget.arts.length));
    }
    //Scroll Start
